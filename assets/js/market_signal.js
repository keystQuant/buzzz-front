(function ($) {
    ...
    $(document).click(function (e) {
        ...
        if( $('.child_filter').hasClass('active') ) {
            if (e.target.className == "child_filter" || $(e.target).parents('.child_filter').length > 0 ) {
                return false;
            } else {
                $('.child_filter').attr('data-click', '0');
                $('.child_filter.active').find('.child_select').css('height', '0px');
                $('.child_filter.active').removeClass('active');
            }
        } else if( $('.cart_portfolio').hasClass('active') ){
            if( e.target.className == "cart_portfolio" || e.target.className == "cart_button" || $(e.target).parents('.cart_portfolio').length > 0 ){
                return false;
            } else {
                $('.cart_button').attr('data-click', '0');
                $('.cart_portfolio.active').removeClass('active');
            }          
        }
    });
